...
const TString fileNames[nSamples] = {"DYJetsToLL_mvaID_90X_9files_v2.root",
				     "DYJetsToLL_mvaID_90X_9files_v2.root",
				     "TTJets_mvaID_90X_9files_v2.root",
				     "TTJets_mvaID_90X_9files_v2.root"};
...
void   getEfficiency(TTree *tree, bool doPU, bool isSignal, bool isBarrel, float &eff, float &effErr);
float  getPUWeight(int nPU, bool doPU);


// Main method
void compareIdElectronCut_v2_PUeffect(){
...
void   getEfficiency(TTree *tree, bool doPU, bool doSignal, bool doBarrel, float &eff, float &effErr){
  ...
  tree->SetBranchAddress("passEleId",&passId, &b_passId );
  ...
  if(verbose>1)
	...
	continue;
      ...
      if( !( ( doBarrel && fabs(eta->at(iEle)) < etaBarrelMax )
	     || (doEndcap && fabs(eta->at(iEle)) > etaEndcapMin && fabs(eta->at(iEle))<etaEndcapMax ) ) )
	continue;

      // Signal or background?
      // signal: 1 gen-matched electrons
      // background: 0 unmatched electrons or 3 true non-prompt electrons (not from taus)
      bool doBackground = !doSignal;
      if( !( (doSignal && isTrue->at(iEle)==1)
	     || ( doBackground && (isTrue->at(iEle) == 0 || isTrue->at(iEle) == 3) ) ) )
	continue;
	...
	printf("   the candidate is to be counted\n");
      ...
      histDen->Fill(pt->at(iEle), weight);

      if( passId->at(iEle) == 1 ){
	// The candidate passed VID ID
	histNum->Fill(pt->at(iEle), weight);
      }
