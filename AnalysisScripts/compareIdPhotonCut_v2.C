...
const TString fileNames[nSamples] = {"Gjet_Pt-20toInf_cutID_80X_small.root",
				     "Gjet_Pt-20toInf_cutID_80X_small.root",
				     "Gjet_Pt-20toInf_cutID_80X_small.root",
				     "Gjet_Pt-20toInf_cutID_80X_small.root"};
...
void compareIdPhotonCut_v2(){
  ...
  for(int iSample = 0; iSample < nSamples; iSample++){
	...
	getEfficiency(trees[iSample][iWP], varNames[iWP], is80X[iSample], isSigOrBg[iSample], isBarrel, 
  ...
  for(int iEta = 0; iEta<nEtaRegions; iEta++){
    ...
    gStyle->SetOptStat(0);
    dummy[iEta] = new TH2F(TString::Format("dummy%d",iEta),"",100,0.5,1,100,0.6,1);
